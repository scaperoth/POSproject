...
class BSHtml extends CHtml {
    ...
    const FORM_LAYOUT_HORIZONTAL_LABEL_CLASS = 'col-lg-4';
    const FORM_LAYOUT_HORIZONTAL_CONTROL_CLASS = 'col-lg-8';
...
//        }
    ...
    protected static function btn($type, $label, $htmlOptions = array()) {
        ...
        $iconColor = \bootstrap\helpers\BSArray::popValue('iconColor', $htmlOptions);
        $iconClass = \bootstrap\helpers\BSArray::popValue('iconClass', $htmlOptions);
        if (!empty($icon)) {
            $label = self::icon($icon, $iconColor, $iconClass) . ' ' . $label;
        }
     ...
     */
    public static function icon($icon, $iconColor='', $iconClass='', $htmlOptions = array(), $tagName = 'i') {
        if (is_string($icon)) {
            if (strpos($icon, 'glyphicon') === false) {
                $icon = 'fa-' . implode(' fa-', explode(' ', $icon));
                self::addCssClass('fa ' . $icon, $htmlOptions);
            }else{
                self::addCssClass('glyphicon ' . $icon, $htmlOptions);
            }
            if (!empty($iconColor) && is_string($iconColor))
                self::addCssStyle('color: ' . $iconColor, $htmlOptions);
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemOptions) {
                ...
                if (is_string($itemOptions)) {
                    ...
                    $iconColor = \bootstrap\helpers\BSArray::popValue('iconColor', $itemOptions);
                    $iconClass = \bootstrap\helpers\BSArray::popValue('iconClass', $itemOptions);
                    if (!empty($icon)) {
                        $label = self::icon($icon, $iconColor, $iconClass) . ' ' . $label;
                    }
    ...
    public static function jumbotron($heading, $content, $htmlOptions = array()) {
        ...
        $output = self::openTag('div', $htmlOptions);
        $output .= self::openTag('div', array('class' => 'container'));
